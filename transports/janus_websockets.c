...
static tracer_track *the_tracer_track;
static tracer_race *the_tracer_race;
...
void tracer_remove_race(char *raceid)
{
    ...
    while (tmp_car_list != NULL)
    {
        ...
        if (car)
        {
            //We remove the driver from our list.
            if (car->driver)
    ...
    case LWS_CALLBACK_RECEIVE:
    {
        ...
        else if (!strcasecmp(command_text, "setdriverofcar"))
        {
            ...
            if (car->driver != NULL)
            {
                ...
                json_object_set_new(message, "session_id", json_integer(tracer_session_id));
                json_object_set_new(message, "handle_id", json_integer(car->driver->handle_id));
                json_t *body = json_object();
                json_object_set_new(body, "request", json_string("stopstream"));
                json_object_set_new(message, "body", body);
