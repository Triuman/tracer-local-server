...
{
	gstreamer_streaming_status_init = 0, //verb, need an action
	gstreamer_streaming_status_inited,   //situation, doesnt need an action
	gstreamer_streaming_status_start,
	gstreamer_streaming_status_started,
	gstreamer_streaming_status_stop,
...
janus_streaming_mountpoint *janus_streaming_create_rtp_source(
		...
		gboolean dovideo, char *vmcast, const janus_network_address *viface, uint16_t vport, uint16_t vrtcpport, uint8_t vcodec, char *vrtpmap, char *vfmtp, char *camera_url, char *gstreamer_pipeline_string, gboolean bufferkf,
...
int janus_streaming_init(janus_callbacks *callback, const char *config_path) {
	...
	if(config != NULL) {
		...
		while(cl != NULL) {
			...
			if(!strcasecmp(type->value, "rtp")) {
				...
				janus_config_item *camera_url = janus_config_get(config, cat, janus_config_type_item, "camera_url");
				janus_config_item *gstreamer_pipeline_string = janus_config_get(config, cat, janus_config_type_item, "gstreamer_pipeline_string");
	...
	{
		g_atomic_int_set(&initialized, 0);
		JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the Gstreamer streaming thread...\n", error->code, error->message ? error->message : "??");
		janus_config_destroy(config);
		return -1;
	}
	...
	if (!strcasecmp(request_text, "sendmessagetodriver"))
	{
		...
		if(!strcasecmp(type_text, "rtp")) {
			...
			mp = janus_streaming_create_rtp_source(
					...
					dovideo, vmcast, &video_iface, vport, vrtcpport, vcodec, vrtpmap, vfmtp, camera_url ? (char *)json_string_value(camera_url) : NULL, gstreamer_pipeline_string ? (char *)json_string_value(gstreamer_pipeline_string) : NULL, bufferkf,
		...
		json_object_set_new(response, "streaming", json_string("ok"));
		goto plugin_response;
	} else if(!strcasecmp(request_text, "acceptanswer") || !strcasecmp(request_text, "acceptoffer")
			|| !strcasecmp(request_text, "startstream") || !strcasecmp(request_text, "stopstream") || !strcasecmp(request_text, "connect")
			|| !strcasecmp(request_text, "watch") || !strcasecmp(request_text, "start")
...
void janus_streaming_incoming_data(janus_plugin_session *handle, char *buf, int len)
{
	...
	*(text + len) = '\0';
	...
	//Command Types
	//- Control: Send this to transport. There, send it to the car controlled by the driver.
	//- ID : Takes this id and send it to transport. There see if this id matches with the one web server gave us. If so, set isValidaded to true. Otherwise hangup connection and remove the driver.
}
...
static void *janus_streaming_handler(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		else if (!strcasecmp(request_text, "acceptoffer"))
		{
			JANUS_LOG(LOG_VERB, "We got a acceptoffer request\n");
			json_t *id = json_object_get(root, "id");
			guint64 id_value = json_integer_value(id);
			janus_mutex_lock(&mountpoints_mutex);
